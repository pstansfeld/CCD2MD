...
def main():
    ...
    parser = argparse.ArgumentParser(description='Convert from output from CHARMM to Martini 3.', add_help=False)
    ...
    parser.add_argument('-C',  '--conc',  help='Concentration of ions in the system - default = 0.15. Default ions are Na and Cl. For membrane embedded proteins, charge balance is maintained; specify different ions via -mem to pass to Insane4MemPrO. For non-membrane proteins charge balance is not maintained; specify different ions via -I.', default=None)
    ...
    if args.title != None:
        title = FuncConv.get_title(args.title)
    ...
    if run_CG_MD:
        ...
        em_name = FuncConv.make_gmx(final, command_line, 'CGEM', args.gmx, ndx=ndx, run=args.run_AA_energy_minimise, topol=topol)
        CG_name = em_name
        ...
        if args.CG_equil:
            ...
            eq_name = FuncConv.make_gmx(em_name, command_line, 'CGeq'+suffix, args.gmx, ndx=ndx, run=args.run_AA_equil, topol=topol)
            CG_name = eq_name
