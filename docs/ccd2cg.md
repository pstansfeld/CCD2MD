# ccd2cg for generating Martini file from co-folding input

ccd2cg can be used to generate a Martini position file from a co-folding input, where a mapping is possible -- optionally with a Martini 3 mapping (see [here](https://github.com/keb721/CCD2MD/blob/main/docs/ligand_table.md) for a list of all mappings which are included by default. Any proteins present are converted via [martinize2](https://github.com/marrink-lab/vermouth-martinize). Any mappings created using the CCD2MD.cif userCCD codes can be converted as well.
...
The use of a configuration file is recommended, an example can be found [here](https://github.com/keb721/CCD2MD/blob/main/docs/ccd2cg_input.txt). The names in the configuration file correspond to the command line arguments.

A schematic for the use of ccd2cg is presented below. The argument -CF can optionally be used to specify the inputs. For more details, please see the full description below.

<p align="center">
  <img width="500" src="ccd2cg.png" alt="ccd2cg workflow">
</p>

## Usage

> ccd2cg INPUT_FILE OUTPUT_FILE [-CF &lt;config_file&gt;] [-T &lt;title&gt;] [-S &lt;smiles&gt; ...] [-nl] [-g &lt;gmx&gt;] [-C &lt;conc&gt;] [(-E [&lt;elastic&gt;]) | (-G &lt;go&gt;)] [-M &lt;martinize&gt;] [-mem [&lt;membrane&gt;]] [-C &lt;conc&gt;] [-mp &lt;mempro&gt;] [-mdef &lt;memprod&gt;] [-ncpu &lt;num_cpus&;] [-B [&lt;box&gt;]] [-SV [&lt;solvate&gt;]] [-I [&lt;ions&gt;]] [-ndx] [-CGEM [&lt;CGEM&gt;]] [-rCGEM [&lt;run_CGEM&gt;]] [-CGeq [&lt;CGeq&gt;]] [-rCGeq [&lt;CGeq&gt;]] [-CGprd [&lt;CGprd&gt;]]
...
`-S/--SMILES <smiles> ...` specifies the name of the SMILES mapping to use (should be added to the database as described in the [CCD2MD paper](https://pubs.acs.org/doi/full/10.1021/acs.jcim.5c02066)). This must specify the number of mappings used, and their order. For multiple of the same ligand, this can be written "POES POES" or "POES 2".
